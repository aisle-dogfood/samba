...
def run_prefork_process(cmd, kwargs, cargs):
	...
	# Decode base64-encoded bytes back to original form
	out = base64.b64decode(out_encoded) if out_encoded is not None else None
	err = base64.b64decode(err_encoded) if err_encoded is not None else None
	if ex:
		if ex == 'OSError':
			raise OSError(trace)
		elif ex == 'ValueError':
