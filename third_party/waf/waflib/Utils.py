...
def run_prefork_process(cmd, kwargs, cargs):
	...
	# Convert back from base64-encoded strings to bytes if needed
	if isinstance(out, dict) and '__bytes__' in out:
		out = base64.b64decode(out['__bytes__'])
	if isinstance(err, dict) and '__bytes__' in err:
		err = base64.b64decode(err['__bytes__'])
	if ex:
		if ex == 'OSError':
			raise OSError(trace)
		elif ex == 'ValueError':
