...
class bld_proxy(object):
		...
		try:
			...
			try:
				...
				try:
					...
					data = self.bld.verify_pickle_data(signed_data)
					if data is None:
						Logs.debug('rev_use: Build cache signature verification failed %s', dbfn)
					else:
	...
	def store(self):
		...
		signed_data = self.bld.sign_pickle_data(x)
