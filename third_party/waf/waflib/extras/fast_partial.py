...
class bld_proxy(object):
		...
		try:
			...
			try:
				...
				try:
					...
					data, is_valid = Build._verify_and_extract_data(signed_data)
					if not is_valid:
						Logs.debug('rev_use: Could not verify the build cache %s (tampered or old format)', dbfn)
					else:
	...
	def store(self):
		...
		signed_data = Build._sign_data(x)
