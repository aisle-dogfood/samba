...
import atexit, errno, getpass, json, os, re, shutil, sys, time, threading, traceback, shlex
try:
...
def cache_command(proc, sig, files_from, files_to):
	...
	command containing the task signature, a list of files to
	cache and a list of files files to get from cache (one of the lists
	is assumed to be empty)
	"""
	obj = json.dumps([sig, files_from, files_to])
...
def loop(service):
	...
	as JSON-encoded commands (one line per command)
	...
	[sig, files_from, files_to] = json.loads(txt)
	if files_from:
