...
def cache_command(proc, sig, files_from, files_to):
	"""
	Create a command for cache worker processes, returns a JSON-serialized
	...
	"""
	obj = base64.b64encode(json.dumps([sig, files_from, files_to]).encode('utf-8'))
	...
	return json.loads(base64.b64decode(obj).decode('utf-8'))
...
def loop(service):
	...
	[sig, files_from, files_to] = json.loads(base64.b64decode(txt).decode('utf-8'))
	if files_from:
	...
	obj = base64.b64encode(json.dumps(ret).encode('utf-8'))
