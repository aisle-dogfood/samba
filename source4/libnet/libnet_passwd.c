...
static NTSTATUS libnet_ChangePassword_samr(struct libnet_context *ctx, TALLOC_CTX *mem_ctx, union libnet_ChangePassword *r)
{
	...
	if (NT_STATUS_IS_OK(status)) {
		 ...
		 * RC4 fallback has been removed for security reasons.
		 * Only secure AES encryption is allowed.
		 */
		r->samr.out.error_string = talloc_asprintf(mem_ctx,
			"Password change failed: Server does not support secure AES encryption. "
			"RC4 fallback is disabled for security reasons.");
