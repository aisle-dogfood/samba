...
static void torture_gnutls_sam_rid_crypt_secure(void **state)
{
	static const uint8_t clear[16] = {
		0x02, 0xFA, 0x3B, 0xEE, 0xE8, 0xBA, 0x06, 0x01,
		0x3F, 0x49, 0x5B, 0x20, 0xA7, 0x84, 0xC2, 0x34
	...
	rc = sam_rid_crypt_secure(rid, clear, crypt, SAMBA_GNUTLS_ENCRYPT);
	assert_int_equal(rc, 0);

	rc = sam_rid_crypt_secure(rid, crypt, decrypt, SAMBA_GNUTLS_DECRYPT);
	assert_int_equal(rc, 0);
	assert_memory_equal(decrypt, clear, 16);

	/* Verify that AES produces different output than DES */
	uint8_t des_crypt[16];
	rc = sam_rid_crypt(rid, clear, des_crypt, SAMBA_GNUTLS_ENCRYPT);
	assert_int_equal(rc, 0);
	
	/* AES and DES should produce different ciphertext */
	assert_memory_not_equal(crypt, des_crypt, 16);
}
...
int main(int argc, char *argv[])
{
	...
	const struct CMUnitTest tests[] = {
		...
		cmocka_unit_test(torture_gnutls_sam_rid_crypt_secure),
