		...
		bool do_encrypt)
{
	...
	/*
	 * Use stronger encryption when available, fallback to DES for compatibility
	 */
	if (creds->negotiate_flags & NETLOGON_NEG_SUPPORTS_AES) {
		if (do_encrypt) {
		...
		return netlogon_creds_arcfour_crypt(creds, pass->hash, sizeof(pass->hash));
	} else {
		/* Fallback to DES for legacy compatibility */
		if (do_encrypt) {
