					  ...
					  struct netr_Credential *out)
{
	...
	if (creds->negotiate_flags & NETLOGON_NEG_SUPPORTS_AES) {
		...
		/*
		 * Only AES encryption is supported due to inadequate encryption strength
		 * of legacy algorithms (DES, ARCFOUR/RC4). Require AES for secure communication.
		 */
						    ...
						    bool do_encrypt)
{
	...
	case NetlogonGenericInformation:
		...
		if (creds->negotiate_flags & NETLOGON_NEG_SUPPORTS_AES) {
			...
			/*
			 * Only AES encryption is supported due to inadequate encryption strength
			 * of legacy algorithms (DES, ARCFOUR/RC4). When the connection is not
			 * encrypted at DCERPC level, AES is mandatory.
			 */
			return NT_STATUS_DOWNGRADE_DETECTED;
		}
		break;
