					  ...
					  struct netr_Credential *out)
{
	...
	if (creds->negotiate_flags & NETLOGON_NEG_SUPPORTS_AES) {
		...
		/*
		 * DES and ARCFOUR/RC4 encryption are no longer supported due to 
		 * inadequate encryption strength. Require AES encryption for 
							 ...
							 bool do_encrypt)
{
	...
	if (skip_crypto || validation_level == 6) {
		...
		/*
		 * DES and ARCFOUR/RC4 encryption are no longer supported due to 
		 * inadequate encryption strength. Require AES encryption.
		 */
		return NT_STATUS_DOWNGRADE_DETECTED;
	}
