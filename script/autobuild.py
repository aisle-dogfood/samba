...
def run_cmd(cmd, dir=".", show=None, output=False, checkfail=True, use_shell=True):
    if show is None:
        do_debug("Running: '%s' in '%s'" % (cmd, dir))
    elif show:
        do_print("Running: '%s' in '%s'" % (cmd, dir))

    # When use_shell=False, parse cmd string into a list of arguments
    if not use_shell:
        ...
        cmd_args = [cmd] if output else cmd

    if output:
        out = check_output(cmd_args, shell=use_shell, cwd=dir)
        return out.decode(encoding='utf-8', errors='backslashreplace')
    elif checkfail:
        return check_call(cmd_args, shell=use_shell, cwd=dir)
    else:
        return call(cmd_args, shell=use_shell, cwd=dir)
...
if status == 0:
    if options.passcmd is not None:
        ...
        run_cmd(options.passcmd, dir=test_master, use_shell=False)
