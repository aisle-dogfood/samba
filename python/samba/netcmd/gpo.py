...
def secure_fromstring(xml_string):
    """
    Secure XML parsing function that prevents XXE attacks by disabling
                ...
                if os.path.isdir(l_name):
                    ...
                    if l_name.endswith('.xml'):
                        ...
                        try:
                            ...
                            concrete_xml = secure_fromstring(data)
                        ...
                        try:
                            with open(l_name, 'r') as ltemp:
                                ...
                                if data.startswith(xml_head):
                                    ...
                                    parser.load_xml(secure_fromstring(xml_head + dtd_header + data))
                                else:
                                    parser.load_xml(secure_fromstring(dtd_header + data))
...
fakeu,fakeg% ALL=(ALL) NOPASSWD: ALL
    ...
    def run(self, gpo, command, user, users, groups=None, passwd=None,
        ...
        try:
            xml_data = ET.ElementTree(secure_fromstring(conn.loadfile(vgp_xml)))
...
samba-tool gpo manage sudoers list {31B2F340-016D-11D2-945F-00C04FB984F9}
    ...
    def run(self, gpo, H=None, sambaopts=None, credopts=None, versionopts=None):
        ...
        try:
            xml_data = secure_fromstring(conn.loadfile(vgp_xml))
        except NTSTATUSError as e:
            if e.args[0] in [NT_STATUS_OBJECT_NAME_INVALID,
                             NT_STATUS_OBJECT_NAME_NOT_FOUND,
                             NT_STATUS_OBJECT_PATH_NOT_FOUND]:
...
samba-tool gpo manage access list {31B2F340-016D-11D2-945F-00C04FB984F9}
    ...
    def run(self, gpo, H=None, sambaopts=None, credopts=None, versionopts=None):
        ...
        try:
            allow = secure_fromstring(conn.loadfile(vgp_xml))
        except NTSTATUSError as e:
            if e.args[0] in [NT_STATUS_OBJECT_NAME_INVALID,
                             NT_STATUS_OBJECT_NAME_NOT_FOUND,
                             NT_STATUS_OBJECT_PATH_NOT_FOUND]:
        ...
        try:
            deny = secure_fromstring(conn.loadfile(vgp_xml))
        except NTSTATUSError as e:
            if e.args[0] in [NT_STATUS_OBJECT_NAME_INVALID,
                             NT_STATUS_OBJECT_NAME_NOT_FOUND,
                             NT_STATUS_OBJECT_PATH_NOT_FOUND]:
