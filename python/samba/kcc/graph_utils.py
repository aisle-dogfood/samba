...
# Whitelist of allowed graph verification functions
# This prevents arbitrary code execution via the properties parameter
...
def verify_graph(edges, vertices=None, directed=False, properties=()):
    ...
    for p in properties:
        ...
        if f is None:
            raise ValueError("Unknown graph property verification: %s" % p)
        try:
