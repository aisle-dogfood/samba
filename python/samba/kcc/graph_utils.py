...
# Static mapping of property names to verification functions
# This prevents arbitrary code execution via dynamic globals() lookup
VERIFY_FUNCTIONS = {
...
def verify_graph(edges, vertices=None, directed=False, properties=()):
    ...
    for p in properties:
        f = VERIFY_FUNCTIONS.get(p)
        if f is None:
            raise ValueError("Unknown verification property: %s" % p)
        try:
...
def list_verify_tests():
    for k, v in sorted(VERIFY_FUNCTIONS.items()):
        print(k)
        if v.__doc__:
